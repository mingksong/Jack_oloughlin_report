      ...
      <div className="inline-block">
        ...
        {grid.map((row, ri) => (
          <div key={ri} className="flex items-center">
            ...
            {row.map((count, ci) => {
              const opacity = count > 0 ? 0.15 + (count / maxCount) * 0.7 : 0.03;
              const pct = total > 0 ? ((count / total) * 100).toFixed(0) : '0';
              return (
                <div
                  key={ci}
                  className="w-16 h-14 flex flex-col items-center justify-center border border-gray-200 rounded"
...
function PitchTypeDistribution({ pitches }: { pitches: AusPitch[] }) {
  ...
  return (
    <div>
      <h4 className="text-xs text-gray-500 font-medium mb-2">결정구 구종</h4>
...
function PutawayResults({ pitches }: { pitches: AusPitch[] }) {
  ...
  const stats = [
    ...
    { label: '인플레이 아웃', count: inPlayOuts, color: '#3b82f6' },
    { label: '피안타', count: hits, color: '#ef4444' },
  ];

  return (
    <div>
      <h4 className="text-xs text-gray-500 font-medium mb-2">결정구 결과</h4>
...
function TwoStrikePitchMap({ pitches }: { pitches: AusPitch[] }) {
  ...
  return (
    <div className="flex flex-col items-center">
      <h4 className="text-xs text-gray-500 font-medium mb-1">결정구 피치맵</h4>
      <svg width={SVG_W} height={SVG_H} className="rounded-lg">
        ...
        <text x={SVG_W / 2} y={SVG_H - 8} textAnchor="middle" fill="#94a3b8" fontSize={10}>
          Pitcher&apos;s View
        </text>
      </svg>
