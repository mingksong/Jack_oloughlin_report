...
function GradeBar({ grade }: { grade: number }) {
  // 20-80 scale → percentage (20=0%, 80=100%)
...
export default function ScoutingGradeTable() {
  ...
  return (
    <div className="space-y-6">
          ...
          {pitches.map(p => (
            <div key={p.code}>
              <div className="flex items-center gap-2 mb-1">
                ...
                />
                <span className="text-xs font-semibold text-white w-28">{p.name}</span>
                <span className="text-[10px] text-slate-400 flex-shrink-0">{p.label}</span>
              </div>
              <GradeBar grade={p.grade} />
              <p className="text-[10px] text-slate-500 mt-0.5 ml-5">{p.desc}</p>
            </div>
          ))}
        </div>

        {/* Grade Scale Legend */}
        <div className="flex justify-between mt-4 pt-3 border-t border-slate-700/50 text-[9px] text-slate-600 px-0">
          <span>20</span>
          <span>30</span>
          <span>40</span>
          <span>50 (Avg)</span>
      ...
      {/* Role Assessment */}
      <div className="bg-slate-800/50 rounded-xl p-5 border border-slate-700/50">
        <h3 className="text-sm font-semibold text-slate-300 mb-3">투수 평가</h3>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="bg-slate-900/50 rounded-lg p-3">
            <span className="text-[10px] text-slate-500 block mb-1">예상 역할</span>
            <span className="text-sm font-bold text-blue-400">{overall.role}</span>
          </div>
          <div className="bg-slate-900/50 rounded-lg p-3">
            <span className="text-[10px] text-slate-500 block mb-1">성장 상한</span>
            <span className="text-sm font-bold text-amber-400">{overall.ceiling}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-slate-800/50 rounded-xl p-4 border border-green-900/30">
          <h4 className="text-xs font-semibold text-green-400 mb-2">강점</h4>
        ...
        <div className="bg-slate-800/50 rounded-xl p-4 border border-yellow-900/30">
          <h4 className="text-xs font-semibold text-yellow-400 mb-2">약점</h4>
          <ul className="space-y-1.5">
        ...
        <div className="bg-slate-800/50 rounded-xl p-4 border border-red-900/30">
          <h4 className="text-xs font-semibold text-red-400 mb-2">리스크</h4>
          <ul className="space-y-1.5">
